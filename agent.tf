# Snowflake Intelligence Agent Definition
# Note: As of Terraform Provider v0.98, there is no native resource for Snowflake Intelligence Agents
# This generates the SQL needed to create the agent, which should be applied via Snowflake CLI or worksheet

locals {
  agent_sql = <<-SQL
    USE DATABASE ${snowflake_database.intelligence.name};
    USE SCHEMA ${snowflake_schema.agents.name};
...
# Write the agent SQL to a local file for easy execution
resource "local_file" "agent_sql" {
  content         = local.agent_sql
  filename        = "${path.module}/generated_agent.sql"
